...
public class OrderServiceImpl implements OrderService {
    ...
    public void deliverOrder(Long orderId) {
        ...
        // COD cash is collected by admin at delivery, so finalize payment before delivery state.
        if (order.getPaymentMethod() == com.shopsphere.orderservice.enums.PaymentMethod.COD) {
            paymentClient.markCodPaymentPaid(orderId);
        }
    ...
    public PaymentResponseDto startPayment(Long orderId, Long userId) {
        ...
        /*
         * Prefer paymentservice's explicit orderStatus. paymentStatus remains the
         * payment ledger result, so COD can be SUCCESS while the order is PAYMENT_DUE.
         */
        if (response != null && response.getOrderStatus() != null && !response.getOrderStatus().isBlank()) {
            updateOrderStatus(orderId, OrderStatus.valueOf(response.getOrderStatus().trim().toUpperCase()));
        } else {
